...
for file in $(find $decodingdir -name "*.wav");do
    echo $file
    ...
    if [ $stage -le 1 ]; then
        ...
        echo java -Xmx2024m -jar $lvcsrRootDir/tools/LIUM_SpkDiarization-8.4.1.jar  \
    ...
    if [ $stage -le 2 ]; then
        ...
        awk '$1 !~ /^;;/ {print $1"-"$8"-"$3/100.0"-"($3+$4)/100.0" "$1" "$3/100.0" "($3+$4)/100.0}' \
		   	$datadir/$fileRootName.seg | sort -nk3 > $datadir/segment
        # Gen utt2spk file
